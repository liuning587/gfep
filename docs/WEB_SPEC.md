...
| **Bridge 状态** | 深度连通性探测；当前仅 **`/api/status`** 暴露是否启用与 `bridge698Host` 配置串 |
...
10. **终端踢下线**（`POST /api/terminals/kick`）：**仅管理员**，合法 `connId` 可断开对应终端连接。
11. 页面与 API **UTF-8** 正常，主流浏览器无 **编码相关** 乱码与前端报错（按 NFR-WEB-UI-003、004）。
...
| 数据注入 | `fep/snapshot_web.go`、`fep/traffic_history.go` | `web.Provider`：主机指标、终端/主站行、流量采样、`KickTerminal` |
