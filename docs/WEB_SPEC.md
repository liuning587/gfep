...
| **普通用户（user）** | 查看系统状态、**在线终端明细**、**APP/主站连接明细**、**698 桥接连接明细**、实时/历史日志、**只读**配置与参数；**无** 黑名单/日志级别/用户管理（除非另行授权，默认关闭） |
...
- 总览页 **宜** 提供入口跳转至 **§3.2 在线终端**、**§3.3 主站/APP**、**§3.4 698 桥接**。
...
**必须** 在控制台中提供 **独立菜单/页面**（与「在线终端」「主站/APP」并列），展示 **桥接链路**：在 **终端侧 TCP 会话** 上挂起的、GFEP 作为客户端 **出站拨号** 至配置 **`BridgeHost698`**（等）的 **第二条 TCP**，用于 698（及实现支持时的其它规约）透传。
...
**当前实现**：`GET /api/bridges?q=`；`web.BridgeRow` JSON；`bridge.Conn` 内统计 **TCP 重连时清零**；`ziface.IConnManager` 提供 **`Range`** 遍历以收集带 `bridge` 属性的连接。前端菜单 **「698 桥接」**。
...
7. **698 桥接** 独立菜单；**§3.4** 字段与 **`/api/bridges`** 一致；无对象时空状态。
