...
func Ptl698_45BuildPacket(tp uint8, tsa []byte) []byte {
	packet := []byte{0x68, 0x1E - 6 + byte(len(tsa)), 0x00, 0x81, byte(len(tsa) - 1)} //, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
	packet2 := []byte{0x00, 0xD2, 0xB6, 0x01, 0x00, tp, 0x01, 0x2C, 0x07, 0xE6, 0x02, 0x17, 0x03, 0x08, 0x06, 0x38, 0x03, 0x22, 0x1C, 0xBC, 0x16}

	getDataTime(packet2[8:])

	packet = append(packet, tsa...)
