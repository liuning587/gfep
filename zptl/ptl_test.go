...
func TestChkfrm_00(t *testing.T) {
	//创建一个报文检测对象
	chkfrm := NewChkfrm(PTL_698_45, 1000, cb, nil)
	...
	rlen, ptype := IsVaild(PTL_698_45, packet)
	fmt.Println("rlen: ", rlen, " ptype: ", ptype)

	cnt = chkfrm.Chkfrm(Str2hex("68"))
	fmt.Println("cnt: ", cnt)
	cnt = chkfrm.Chkfrm(Str2hex("21 00 C3 05 11 11 11 11 01 00 CC 38 81"))
	fmt.Println("cnt: ", cnt)
	cnt = chkfrm.Chkfrm(Str2hex("85 01 01 40 00 02 00 01 1C 07 E4 04 03 0F 02 0A 00 00 72 99 16"))
	fmt.Println("cnt: ", cnt)
}
